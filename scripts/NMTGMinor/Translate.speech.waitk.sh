...
if [ $BEAMSIZE == 1 ]; then
    out=$BASEDIR/data/$name/eval/$set.prefix.from.$prefix_from.rate.$rate.t
else
    out=$BASEDIR/data/$name/eval/$set.beam$BEAMSIZE.prefix.from.$prefix_from.rate.$rate.t
fi
