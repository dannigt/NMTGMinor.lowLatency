...
class EnsembleTranslator(object):
    ...
    def translate_batch(self, batch, length_batch=None):
        ...
        # commit_depth = int(self.opt.commit_depth) if self.opt.commit_depth is not None else -1
        # decoding_buffer_depths = self.opt.decoding_buffer_depths
        ...
        for current_depth in range(self.opt.max_sent_length):
            ...
            for k in range(self.n_models):
                ...
                decoder_output = self.models[k].step(decoder_input.clone(), decoder_states[k], current_depth)
            ...
            for seq_idx in range(batch_size):
                ...
                if self.force_target_length and length_batch and length_batch[seq_idx] == current_depth:
            ...
            # max_idx = beam[0].getCurrentState().data[0].item()
            # print(max_idx)
            # print('---------------', torch.max(torch.exp(word_lk[0][0])))
            # print(self.tgt_dict.convertToLabels([max_idx], onmt.Constants.EOS))
            # , self.build_target_tokens(max_idx))
        ...
        for seq_idx in range(batch_size):
            ...
            hyps, attn, length = zip(*[beam[seq_idx].getHyp(k) for k in ks[:n_best]])
            all_hyp += [hyps]
            all_lengths += [length]
    ...
    def translate_asr(self, src_data, tgt_data, length=None):
        ...
        #  (2) translate
        pred, pred_score, attn, pred_length, gold_score, gold_words, allgold_words, all_lk = self.translate_batch(batch, length)
